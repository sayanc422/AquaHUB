...
deploy() {
  ...
  if [[ "$PROFILE" == commerce || "$PROFILE" == full-app ]]; then
    ...
    # order-service will not start without the tank-enquiry encryption key, and
    # that Secret is deliberately not in this repository -- it is the one secret
    # here that is not plaintext in Git (ADR 0021). Checked before applying
    # anything, because the alternative is a 300 s rollout timeout further down
    # that says nothing about the cause.
    #
    # This script does NOT create it. A key generated by a bootstrap script is a
    # key nobody wrote down, and the first `--destroy` would take every stored
    # enquiry with it.
    if ! kubectl -n "$NS" get secret order-inquiry-key >/dev/null 2>&1; then
      warn "Secret order-inquiry-key is missing: the custom-tank enquiry form will answer 503 until
   you create it. order-service, checkout and everything else roll out normally -- the missing key
   disables one endpoint, not the service (ADR 0021). Create it with:

     kubectl create secret generic order-inquiry-key --namespace ${NS} \\
       --from-literal=INQUIRY_ENCRYPTION_KEY=\"\$(openssl rand -base64 48)\"

   Keep a copy outside the cluster -- there is no rotation and no recovery.
   See docs/runbooks/rotate-or-create-the-inquiry-key.md"
    fi
